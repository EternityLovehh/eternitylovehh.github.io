...
基于对真实社区与市场平台（Reddit/ProAndroidDev、牛客网、知乎、掘金、CSDN、GitHub 面试题库、CoderScreen、各公司招聘指南）的检索，下面是 AI 盛行时代 Android
  ...
  面试官实际在评什么（这些是会写进评分表的维度）：
  - Prompt 能力：能不能把模糊需求拆成 AI 能干的明确子任务
  - Output validation：AI 给的代码你能不能一眼看出哪里错、边界没覆盖
  ...
  ▎ 备战动作：练习边用 AI 边解说自己的判断。把"我为什么接受/拒绝这段 AI 输出"说出口，这是新的区分度。
  ...
  1. 结构化并发：什么是 structured concurrency？
  → 追问：父协程为什么必须等所有子协程完成？
  → 追问：一个子协程抛异常，默认会发生什么？（答：取消父和所有兄弟协程）
  → 追问：那我不想让一个子失败拖垮其他怎么办？（答：supervisorScope / SupervisorJob）
  2. 取消机制：协程取消是什么特性？
  → 答到点：取消是**协作式（cooperative）**的，Job 被 cancel 后，协程要在挂起点或主动检查（isActive / ensureActive / yield）时才真正停
  → 追问：你写了个 while 循环做计算，cancel 不掉，怎么办？
  → 追问：在 finally 里还要做挂起调用（如关资源）怎么办？（答：withContext(NonCancellable)）
  3. viewModelScope 为什么防泄漏：
  → 答到点：ViewModel onCleared() 时自动 cancel 该 scope 下所有协程
  4. Dispatchers：IO / Default / Main 各自场景？Dispatchers.IO 底层线程池怎么回事？
  5. exception handling：try-catch 抓不到 launch 里的异常？CoroutineExceptionHandler 用在哪？async 的异常什么时候抛？
  ...
  - derivedStateOf 解决什么问题？什么时候必须用它？（答：派生状态、避免不必要重组）
  - 状态提升（state hoisting）：为什么要把 state 提到父级？无状态 composable 好处？（可测、可复用、单向数据流）
  ...
  - Compose 的导航（NavHost）怎么传参、怎么传复杂对象（不建议传大对象，传 ID）？

  ---
  五、架构（中高级必问）

  - MVVM vs MVI vs MVP 三者取舍，为什么新项目倾向 MVI？
  → MVI 答到点：单一不可变 state、单向数据流、事件可追溯、易测试
  - UDF（单向数据流） 怎么落地？
  - ViewModel 如何在屏幕旋转中存活？SavedStateHandle 解决什么？
  - 依赖注入：Hilt 的 scope（@Singleton/@ViewModelScoped）？（注意：你们项目用 DSpi，不用 Hilt —— 但面试市场普遍考 Hilt/Dagger）
  ...
  答题必须覆盖的维度（面试官的 checklist）：

  1. 网络协议选型：REST / WebSocket / SSE / Push，为什么选这个
  2. 存储：关系型(Room) / 文件 / KV(MMKV/DataStore)，怎么选
  3. Offline-First：
    - 本地 DB 作为 single source of truth
    - Room 既是缓存也是离线操作队列（用户操作先写本地，联网再同步）
  4. 同步与幂等：
    - 重试不能产生重复 → 客户端生成 request ID + 服务端去重 + Idempotency-Key 头
    - push 同步 vs pull 同步取舍
  5. 冲突解决：last-write-wins / 版本号 / 复杂场景 OT
  6. 可靠后台任务：WorkManager + Room + NetworkCallback 组合
  7. 缓存策略：内存 + 磁盘多级、缓存失效、预取分页
  ...
  4. 耗电优化：唤醒锁、定位频率、WorkManager 约束、Doze 模式

  5. APK 瘦身：资源混淆、R8/ProGuard、so 拆分(abi split)、App Bundle、图片 webp/无用资源
  ...
  - 设计取舍能力：随着 AI 接管常规活，架构判断力被下放到每个层级，不再只是 Staff 的事
  ...
  给准备者的真实建议：八股要懂原理（不是背），重点投入系统设计（海外）或性能优化+源码（国内），并刻意练习"如何用 AI 高效解题且能讲清自己的判断"——这是 AI
  时代区别于纯背题选手的关键。
